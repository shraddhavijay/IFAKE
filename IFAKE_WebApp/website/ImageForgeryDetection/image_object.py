...
import website.ImageForgeryDetection.container as cont
import website.ImageForgeryDetection.blocks as blk
...
class image_object(object):
    def __init__(self, imageDirectory, imageName, blockDimension, outputDirectory):
        ...
        self.featurescontainer = cont.container()
        self.blockPaircontainer = cont.container()
        ...
        if self.isThisRGBImage:
            for i in tqdm(range(0, imageWidthOverlap + 1, 1)):
                for j in range(0, imageHeightOverlap + 1, 1):
                    ...
                    imageBlock = blk.blocks(imageBlockGrayscale, imageBlockRGB, i, j, self.blockDimension)
            ...
            for i in range(imageWidthOverlap + 1):
                for j in range(imageHeightOverlap + 1):
                    ...
                    imageBlock = blk.blocks(imageBlockGrayscale, None, i, j, self.blockDimension)
                    self.featurescontainer.addBlock(imageBlock.computeBlock())
